...
module.exports = function(settings) {
  ...
  return function(sourceBundle, context, callback) {
    ...
    async.series([
      function(cb) {
        ...
        settings.logger.info("making temp directory %s", extractDir);
        fs.mkdir(extractDir, cb);
      },
      function(cb) {
        settings.logger.info("extract source bundle");
        extractBundle(sourceBundle.readStream, extractDir, cb);
      },
      function(cb) {
        settings.logger.info("get version manifest");
      ...
      function(cb) {
        ...
        settings.logger.info("create new app version");
      ...
      settings.logger.info("Cleanup extracted source archive");
  ...
  function extractBundle(tarStream, extractDir, callback) {
    var error = false;
    settings.logger.info("decompressing and extracting tar bundle to %s", extractDir);

    var extractor = tar.Extract({ path: extractDir, strip: 1 })
      .on('error', function(err) {
        error = true;
        settings.logger.error("error unzipping archive: %o", err);
        return callback(err);
      })
      .on('end', function() {
        settings.logger.info("done unzipping archive");
    ...
    var gunzip = zlib.createGunzip()
      .on('error', function(err) {
        error = true;
        settings.logger.error("error gunzipping: %s", err.stack);
        return callback(err);
      })
      .on('end', function() {
  ...
  function deployFiles(rootDir, appId, versionId, callback) {
    ...
    readdirp(options, function() {}, function(err, res) {
      ...
      settings.logger.info("starting deployment of %s files", res.files.length);

      // Deploy each file
      async.each(res.files, function(fileInfo, cb) {
        fileInfo.size = fileInfo.stat.size;

        deploy(appId, versionId, fileInfo, cb);
      }, function(err) {
        if (err) return callback(err);

        settings.logger.info("done deploying files");
        callback();
      });
    });
  }
