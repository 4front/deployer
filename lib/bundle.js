  ...
  return function(sourceBundle, context, callback) {
    ...
    async.series([
      function(cb) {
        ...
        db.listVersions(appId, {excludeIncomplete: false}, function(err, existingVersions) {
          if (err) return cb(err);

          if (some(existingVersions, {commit: sourceBundle.commit})) {
          ...
          if (some(existingVersions, {status: 'initiated'})) {
      ...
      function(cb) {
        ...
        debug('download package.json manifest');
        downloadManifest(appId, deployedVersion.versionId, function(err, manifestJson) {
          ...
          if (isFunction(context.onManifest)) {
      ...
      function(cb) {
        if (versionError) return cb();

        // Seems counter-intuitive, but make sure the deployment runs for
        // at least 15 seconds to make the dashboard seem more impressive
        // so the spinner can display for at least a few seconds.
      ...
      function(cb) {
        assign(deployedVersion, {
          fileCount: sourceBundle.fileCount,
          virtualEnv: sourceBundle.virtualEnv
        });

        if (versionError) {
          if (versionError.log !== false) {
