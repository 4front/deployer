  ...
  return function(sourceBundle, context, callback) {
    ...
    async.series([
      ...
      function(cb) {
        ...
        var buildEngine;
        try {
          buildEngine = require('../engines/' + sourceBundle.buildConfig.engine)(settings);
        } catch (requireError) {
          return cb(new Error('Invalid build engine ' + sourceBundle.buildConfig.engine));
        }

        buildEngine(sourceBundle, deployedVersion.versionId, appId, function(err) {
          if (err) {
            versionError = err;
            return cb();
          }
