...
module.exports = function(settings) {
  ...
  // exports.updateStatus = function(versionData, virtualApp, callback) {
  //   var virtualEnv = versionData.virtualEnv || 'production';
  //   versionData = pick(versionData, 'status', 'versionId', 'error',
  //     'manifest', 'duration', 'fileCount');
  //
  //   debug('updating version status to %s', versionData.status);
  //
  ...
  //     db.updateTrafficRules(versionData.appId, virtualEnv, trafficRules, function(innerErr) {
  //       if (err) {
  ...
  exports.ensureCanBeDeployed = function(params, callback) {
    ...
    db.listVersions(params.appId, {excludeIncomplete: false}, function(err, existingVersions) {
      ...
      if (some(existingVersions, {status: 'initiated'})) {
        return callback(Error.create('There is already a deployment of this app in-progress.', {
