...
module.exports = function(settings) {
  ...
  exports.create = function(params, callback) {
    ...
    var versionData = pick(params, 'appId', 'versionId', 'name', 'message',
      'manifest', 'commit', 'fileCount', 'hasLog', 'status', 'userId',
      'startedAt', 'queuedAt');
    ...
    tasks.push(function(cb) {
      debug('get next version num for appId %s', versionData.appId);
  ...
  exports.delete = function(appId, versionId, callback) {
    settings.database.getVersion(appId, versionId, function(err, version) {
      ...
      deleteVersion(appId, versionId, callback);
    });
  };

  // Delete all the versions for an application
  exports.deleteAll = function(appId, callback) {
  ...
  exports.deleteOldest = function(appId, trafficRules, numberToDelete, callback) {
    ...
    async.series([
      function(cb) {
        ...
        db.listVersions(appId, opts, function(err, data) {
          if (err) return cb(err);
      ...
      function(cb) {
        async.each(versionsToDelete, function(versionId, done) {
          ...
          deleteVersion(appId, versionId, done);
        }, cb);
      }
    ], callback);
  };

  // Cancel all running versions
  exports.cancelAll = function(params, callback) {
    var db = settings.database;
    async.waterfall([
      function(cb) {
        db.listVersions(params.appId, {excludeIncomplete: false}, cb);
      },
      function(versions, cb) {
        async.each(versions, function(version, next) {
          if (version.status === 'queued') {
            // If the version is queued, delete it.
            debug('Delete queued version %s', version.versionId);
            deleteVersion(params.appId, version.versionId, cb);
          } else if (version.status === 'running') {
            debug('Force cancellation of version %s', version.versionId);
            db.updateVersion({
              appId: params.appId,
              versionId: version.versionId,
              status: 'failed',
              error: 'Cancelled',
              duration: Date.now() - version.startedAt
            }, next);
          } else {
            next();
          }
